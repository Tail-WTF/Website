<script lang="ts">
  ...
  type PageState =
    ...
    | { mode: "success"; text: string; sanitizedURL: string }
    | { mode: "error"; originalUrl: string };
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    const urlToSanitize = inputUrl;
    pageState = { mode: "loading", url: urlToSanitize };

    try {
      const params = new URLSearchParams({ text: urlToSanitize });
      ...
      if (data.sanitizedURLs?.[0]) {
        ...
        pageState = { mode: "error", originalUrl: urlToSanitize };
      }
    } catch {
      pageState = { mode: "error", originalUrl: urlToSanitize };
    }
  ...
  <Layout paddingTop={false}>
    ...
    <div class="mt-20">
      ...
      <p class="mt-7 text-justify hyphens-auto md:w-4/6">
        ...
        Workers for our API, Workers AI for intelligent URL analysis, and Browser
        Rendering for verifying page content. Your requests are processed through
        Cloudflare's infrastructure. For details on how Cloudflare handles data, please
        see
        <a
...
{:else if pageState.mode === "success"}
  <ResultSuccess
    text={pageState.text}
    sanitizedURL={pageState.sanitizedURL}
    onReset={handleReset}
  />
{:else}
  <ResultFailure originalUrl={pageState.originalUrl} onReset={handleReset} />
{/if}
