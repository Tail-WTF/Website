<script lang="ts">
  ...
  type Link = { original: string; sanitized: string; method: string };

  type PageState =
    | { mode: "input" }
    | { mode: "loading"; text: string }
    | { mode: "success"; text: string; links: Link[] }
    | { mode: "error"; unsanitized: string[] };

  let pageState = $state<PageState>({ mode: "input" });
  let inputUrl = $state("");
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    const textToSanitize = inputUrl;
    pageState = { mode: "loading", text: textToSanitize };

    try {
      const query = `text=${encodeURIComponent(textToSanitize)}`;
      const res = await fetch(`${apiFor("/sanitize")}?${query}`, {
        signal: AbortSignal.timeout(30_000),
      });
      const data: { text: string; links: Link[] } = await res.json();

      const changed = data.links?.find((link) => link.method !== "none");
      if (changed) {
        pageState = {
          mode: "success",
          text: data.text,
        ...
        pageState = {
          mode: "error",
          unsanitized: data.links?.map((link) => link.original) ?? [],
        };
      }
    } catch {
      pageState = { mode: "error", unsanitized: [] };
    }
  }

  function handleReset() {
    pageState = { mode: "input" };
    inputUrl = "";
  }
  ...
  async function handleInputClick() {
    if (inputUrl === "") {
      try {
        inputUrl = await navigator.clipboard.readText();
      } catch {
...
<svelte:head>
  {#if pageState.mode === "success"}
    <title>Result - Tail.WTF</title>
  {:else if pageState.mode === "error"}
    <title>Rule Missing - Tail.WTF</title>
  {:else if pageState.mode === "loading"}
  ...
  <Layout paddingTop={false}>
      ...
      <div class="mt-10">
        <form onsubmit={handleSubmit}>
          <LinkBox
            ...
            onclick={inputUrl === "" ? handleInputClick : undefined}
            autofocus
            enterkeyhint="go"
          />
          <ActionBtn
    ...
    <div class="mt-20">
      ...
      <p class="mt-7 text-justify hyphens-auto md:w-4/6">
        ...
        share buttons, innocent-looking random strings are placed in the links
        you get - they are actually
      ...
      <p class="mt-7 text-justify hyphens-auto md:w-4/6">
        ...
        <br />
        <br />
        This site is proxied through and powered by Cloudflare. We use Cloudflare
        Workers for our API. When you explicitly request live-page verification, our
        server fetches the link's page to compare its content with and without parameters;
        nothing is fetched otherwise. For details on how Cloudflare handles data,
        please see
        <a
...
{:else if pageState.mode === "success"}
  <ResultSuccess
    text={pageState.text}
  ...
  />
{:else}
  <ResultFailure unsanitized={pageState.unsanitized} onReset={handleReset} />
{/if}
